...
class HttpRespResult<T> constructor(
    ...
    override fun send(): HttpResult<T> {
        ...
        return HttpResult(
            ...
            httpResp.headers,
            httpResp.url,
            resDataConverter.convert(respAction.invoke(httpResp.res), respType),
            httpResp.err
        )
    }
}
...
class HttpStreamResult(
    ...
    override fun download(
        exc: ((e: Throwable) -> Unit)?,
        download: ((downState: DownState) -> Unit)?
    ) {
        val httpResp = execute()
        if (httpResp.isSuccessful) {
            try {
                downloadHandler.saveFile(
                    httpResp.inputStream!!,
                    downParam,
