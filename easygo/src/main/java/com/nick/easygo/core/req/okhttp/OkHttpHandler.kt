...
class OkHttpHandler : HttpHandler {
    ...
    private fun proceed(httpReq: HttpReq): Request {
        val jsonBody =
            httpReq.httpReqBody.jsonString.toRequestBody("Content-Type:application/json;charset=utf-8".toMediaTypeOrNull())
        val body =
            if (httpReq.httpReqBody.isMultiPart) multiPart(httpReq.httpReqBody.multipartBody) else form(
                httpReq.httpReqBody.fieldMap
            )
    ...
    private fun multiPart(multipartBodyMap: Map<String, Any>): MultipartBody {
        ...
        multipartBodyMap.forEach {
            if (it.value is String) {
                ...
                val requestBody =
                    (it.value as File).asRequestBody("Content-Type: application/octet-stream".toMediaTypeOrNull())
