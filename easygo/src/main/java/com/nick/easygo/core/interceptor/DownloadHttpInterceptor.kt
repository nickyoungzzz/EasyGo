...
internal class DownloadHttpInterceptor(private val breakPoint: Boolean, private val range: Long) :
    HttpInterceptor {
    ...
    override fun intercept(chain: HttpInterceptor.Chain): HttpResp {
        ...
        if (breakPoint && range > 0) {
            return chain.proceed(
                originalHttpReq.newBuilder().addHeader("Range", "bytes=${range}-").build()
            )
        }
