...
class HttpEmitter internal constructor(private val param: HttpParam) {
    ...
    internal fun generateHttpReq(): HttpReq {
        return HttpReq(
            param.url,
            param.reqMethod,
            this@HttpEmitter.reqTag,
            param.headerMap,
            param.queryMap,
            HttpReqBody(param.fieldMap, param.multipartBody, param.isMultiPart, param.jsonString),
            this@HttpEmitter.asDownload
    ...
    inline fun <reified T> asResult(noinline respAction: (String?) -> String? = { it }): HttpRespResult<T> {
        return HttpRespResult(
            generateRealHttpEmitter(),
            this.httpConfig.resDataConverter,
            object : TypeTaken<T>() {}.type,
            respAction
        )
    }
